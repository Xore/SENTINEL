...
type Config struct {
	...
	TLSKeyFile          string
	ClientCAFile        string
	CollectorCACertFile string
	CollectorCAKeyFile  string
	VMOTLPURL           string
	DatabaseURL         string
	MaxMessageBytes     int
...
func Load() (Config, error) {
	cfg := Config{
		...
		CollectorCACertFile: os.Getenv("SENTINEL_COLLECTOR_CA_CERT_FILE"),
		CollectorCAKeyFile:  os.Getenv("SENTINEL_COLLECTOR_CA_KEY_FILE"),
	...
	if cfg.CollectorCACertFile == "" || cfg.CollectorCAKeyFile == "" {
		return Config{}, errors.New(
			"SENTINEL_COLLECTOR_CA_CERT_FILE and SENTINEL_COLLECTOR_CA_KEY_FILE are required",
		)
	}
